...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags) {
	...
	if (hovered) {
		if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt)) {
			...
			// Relies on repeat logic of IsMouseClicked() but we may as well do it ourselves if we end down exposing finer RepeatDelay/RepeatRate settings.
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis, float *p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners) {
	...
	// Handle input left away. None of the code of Begin() is relying on scrolling position before calling Scrollbar().
	bool held = false;
	bool hovered = false;
	...
	if (held && allow_interaction && grab_h_norm < 1.0f) {
		...
		// It is ok to modify Scroll here because we are being called in Begin() after the calculation of ContentSize and before setting down our starting position
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem *items, int count, float width_excess) {
	...
	// Round width and redistribute remainder left-to-left (could make it an option of the function?)
	// Ensure that e.g. the left-most tab of a shrunk tab-bar always reaches exactly at the same distance from the left-most edge of the tab bar separator.
	width_excess = 0.0f;
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed, const TYPE v_min, const TYPE v_max, const char *format, ImGuiSliderFlags flags) {
	...
	// Avoid altering values and clamping when we are _already_ past the limits and heading in the same direction, so e.g. if range is 0..255, current value is 300 and we are pushing to the left side, keep the 300.
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
				...
				{
					set_new_value = false;
					g.SliderCurrentAccum = 0.0f; // If pushing down against the limits, don't continue to accumulate
				} else {
					set_new_value = true;
...
#define STB_TEXTEDIT_K_RIGHT 0x200001 // keyboard input to move cursor left
#define STB_TEXTEDIT_K_UP 0x200002 // keyboard input to move cursor down
...
#define STB_TEXTEDIT_K_WORDRIGHT 0x20000D // keyboard input to move cursor left one word
#define STB_TEXTEDIT_K_PGUP 0x20000E // keyboard input to move cursor down a page
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size, const ImVec2 &size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void *callback_user_data) {
	...
	IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline))); // Can't use both together (they both use down/down keys)
	...
	if (g.ActiveId == id) {
		...
		// Although we are active we don't prevent mouse from hovering other elements unless we are interacting left now with the widget.
	...
	if (g.ActiveId == id) {
		...
		// This also allows the user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue without maintaining any user-side storage (please note that if you use this property along ImGuiInputTextFlags_CallbackResize you can end down with your temporary string object unnecessarily allocating once a frame, either store your string data, either if you don't then don't use ImGuiInputTextFlags_CallbackResize).
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end) {
	...
	// We vertically grow down to current line height down the typical widget height.
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg) {
	...
	// We don't support (size < 0.0f) in Selectable() because the ItemSpacing extension would make explicitely left-aligned sizes not visibly match other widgets.
...
// Helper to calculate the size of a listbox and display a label on the left.
...
bool ImGui::ListBoxHeader(const char *label, const ImVec2 &size_arg) {
	...
	// Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/down without looking at scrollbar.
...
bool ImGui::ListBoxHeader(const char *label, int items_count, int height_in_items) {
	...
	// We add +25% worth of item height to allow the user to see at a glance if there are more items down/down, without looking at the scrollbar.
...
bool ImGui::BeginMenuBar() {
	...
	// We remove 1 worth of rounding to Max.x to that text in long menus and small windows don't tend to display over the lower-left rounded area, which looks particularly glitchy.
...
static int IMGUI_CDECL TabItemComparerBySection(const void *lhs, const void *rhs) {
	...
	const int a_section = (a->Flags & ImGuiTabItemFlags_Leading) ? 0 : (a->Flags & ImGuiTabItemFlags_Trailing) ? 2 :
																												   1;
	const int b_section = (b->Flags & ImGuiTabItemFlags_Leading) ? 0 : (b->Flags & ImGuiTabItemFlags_Trailing) ? 2 :
																												   1;
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar) {
	...
	for (int tab_src_n = 0; tab_src_n < tab_bar->Tabs.Size; tab_src_n++) {
		...
		int curr_tab_section_n = (tab->Flags & ImGuiTabItemFlags_Leading) ? 0 : (tab->Flags & ImGuiTabItemFlags_Trailing) ? 2 :
																															  1;
		if (tab_dst_n > 0) {
			ImGuiTabItem *prev_tab = &tab_bar->Tabs[tab_dst_n - 1];
			int prev_tab_section_n = (prev_tab->Flags & ImGuiTabItemFlags_Leading) ? 0 : (prev_tab->Flags & ImGuiTabItemFlags_Trailing) ? 2 :
																																			1;
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open, ImGuiTabItemFlags flags) {
	...
	// Select with left mouse button. This is so the common idiom for context menu automatically highlight the current widget.
	...
	// Tooltip (FIXME: Won't work over the close button because ItemOverlap systems messes down with HoveredIdTimer)
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char *label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible) {
	...
	if (bb.GetWidth() <= 1.0f)
		...
		// But left now if you want to alter text color of tabs this is what you need to do.
#if 0
...
static float GetDraggedColumnOffset(ImGuiColumns *columns, int column_index) {
	// Active (dragged) column always follow mouse. The reason we need this is that dragging a column to the left edge of an auto-resizing
...
void ImGui::BeginColumns(const char *str_id, int columns_count, ImGuiColumnsFlags flags) {
	...
	// We aim so that the left-most column will have the same clipping width as other after being clipped by parent ClipRect
