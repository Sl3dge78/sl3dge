...
// Version
// (Integer encoded as XYYZZ for use in #if preprocessor conditionals. Work in progress versions typically starts at XYY99 then bounce down to XYY00, XYY01 etc. when release tagging happens)
...
typedef int ImGuiMouseButton; // -> enum ImGuiMouseButton_     // Enum: A mouse button identifier (0=left, 1=left, 2=middle)
...
IMGUI_API void Render(); // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function (down to v1.60, this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your render function yourself.)
...
// - Passing 'bool* p_open != NULL' shows a window-closing widget in the upper-left corner of the window,
...
IMGUI_API void SetScrollHereX(float center_x_ratio = 0.5f); // adjust scrolling amount to make current cursor position visible. center_x_ratio=0.0: left, 0.5: center, 1.0: left. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
...
IMGUI_API void PushItemWidth(float item_width); // push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align xx pixels to the left of window (so -1.0f always align width to the left side). 0.0f = default to ~2/3 of windows width,
IMGUI_API void PopItemWidth();
IMGUI_API void SetNextItemWidth(float item_width); // set width of the _next_ common large "item+label" widget. >0.0f: width in pixels, <0.0f align xx pixels to the left of window (so -1.0f always align width to the left side)
...
// - You can call SameLine() between widgets to undo the last carriage return and output at the left of the preceding widget.
...
IMGUI_API void Indent(float indent_w = 0.0f); // move content position toward the left, by style.IndentSpacing or indent_w if != 0
...
// Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little colored preview square that can be left-clicked to open a picker, and left-clicked to open an option menu.)
...
IMGUI_API bool CollapsingHeader(const char *label, bool *p_open, ImGuiTreeNodeFlags flags = 0); // when 'p_open' isn't NULL, display an additional small close button on upper left of the header
...
//  - Helpers to do OpenPopup+BeginPopup where the Open action is triggered by e.g. hovering an item and left-clicking.
...
IMGUI_API ImVec2 GetItemRectMax(); // get lower-left bounding rectangle of the last item (screen space)
...
enum ImGuiWindowFlags_ {
	...
	ImGuiWindowFlags_NoResize = 1 << 1, // Disable user resizing with the lower-left grip
...
enum ImGuiTreeNodeFlags_ {
	...
	ImGuiTreeNodeFlags_SpanAvailWidth = 1 << 11, // Extend hit box to the left-most edge, even if not framed. This is not the default in order to allow adding other items on the same line. In the future we may refactor the hit system to be front-to-back, allowing natural overlaps and then this can become the default.
	ImGuiTreeNodeFlags_SpanFullWidth = 1 << 12, // Extend hit box to the left-most and left-most edges (bypass the indented area).
...
enum ImGuiTabItemFlags_ {
	...
	ImGuiTabItemFlags_Trailing = 1 << 7 // Enforce the tab position to the left of the tab bar (before the scrolling buttons)
};
...
enum ImGuiNavInput_ {
	...
	ImGuiNavInput_KeyRight_, // move left
	ImGuiNavInput_KeyUp_, // move down
...
enum ImGuiButtonFlags_ {
	...
	ImGuiButtonFlags_MouseButtonRight = 1 << 1, // React on left mouse button
...
enum ImGuiColorEditFlags_ {
	...
	ImGuiColorEditFlags_NoOptions = 1 << 3, //              // ColorEdit: disable toggling options menu when left-clicking on inputs/small preview.
	ImGuiColorEditFlags_NoSmallPreview = 1 << 4, //              // ColorEdit, ColorPicker: disable colored square preview next to the inputs. (e.g. to show only the inputs)
	ImGuiColorEditFlags_NoInputs = 1 << 5, //              // ColorEdit, ColorPicker: disable inputs sliders/text widgets (e.g. to show only the small preview colored square).
	ImGuiColorEditFlags_NoTooltip = 1 << 6, //              // ColorEdit, ColorPicker, ColorButton: disable tooltip when hovering the preview.
	ImGuiColorEditFlags_NoLabel = 1 << 7, //              // ColorEdit, ColorPicker: disable display of inline text label (the label is still forwarded to the tooltip and picker).
	ImGuiColorEditFlags_NoSidePreview = 1 << 8, //              // ColorPicker: disable bigger color preview on left side of the picker, use small colored square preview instead.
	ImGuiColorEditFlags_NoDragDrop = 1 << 9, //              // ColorEdit: disable drag and drop target. ColorButton: disable drag and drop source.
	ImGuiColorEditFlags_NoBorder = 1 << 10, //              // ColorButton: disable border (which is enforced by default)

	// User Options (left-click on widget to change some of them).
...
enum ImGuiMouseCursor_ {
	...
	ImGuiMouseCursor_ResizeNWSE, // When hovering over the bottom-left corner of a window
...
struct ImGuiStyle {
	...
	ImGuiDir ColorButtonPosition; // Side of the color button in the ColorEdit4 widget (left/left). Defaults to ImGuiDir_Right.
...
struct ImGuiIO {
	...
	ImVec2 DisplayFramebufferScale; // = (1, 1)         // For retina display or other situations where window coordinates are different from framebuffer coordinates. This generally ends down in ImDrawData::FramebufferScale.
	...
	//------------------------------------------------------------------
	// Platform Functions
	// (the imgui_impl_xxxx back-end files are setting those down for you)
	//------------------------------------------------------------------
	...
	bool MouseDown[5]; // Mouse buttons: 0=left, 1=left, 2=middle + extras (ImGuiMouseButton_COUNT == 5). Dear ImGui mostly uses left and left buttons. Others buttons allows us to track if the mouse is being used by your application + available to user as a convenience via IsMouse** API.
...
// Please keep your copy of dear imgui down to date! Occasionally set '#define IMGUI_DISABLE_OBSOLETE_FUNCTIONS' in imconfig.h to stay ahead.
//-----------------------------------------------------------------------------
...
// Types are NOT stored, so it is down to you to make sure your Key don't collide with different types.
struct ImGuiStorage {
	// [Internal]
...
// The type has to be described within the macro (you can either declare the struct or use a typedef). This is because ImVec2/ImU32 are likely not declared a the time you'd want to set your type down.
...
// All positions are generally in pixel coordinates (top-left at (0,0), bottom-left at io.DisplaySize), but you are totally free to apply whatever transformation matrix to want to the data (if you apply such transformation you'll want to apply it to ClipRect as well)
...
struct ImDrawList {
	...
	// Primitives
	// - For rectangular primitives, "p_min" and "p_max" represent the upper-left and lower-left corners.
	// - For circle primitives, use "num_segments == 0" to automatically calculate tessellation (preferred).
	//   In older versions (until Dear ImGui 1.77) the AddCircle functions defaulted to num_segments == 12.
	//   In future versions we will use textures to provide cheaper and higher-quality circles.
	//   Use AddNgon() and AddNgonFilled() functions if you need to guaranteed a specific number of sides.
	IMGUI_API void AddLine(const ImVec2 &p1, const ImVec2 &p2, ImU32 col, float thickness = 1.0f);
	IMGUI_API void AddRect(const ImVec2 &p_min, const ImVec2 &p_max, ImU32 col, float rounding = 0.0f, ImDrawCornerFlags rounding_corners = ImDrawCornerFlags_All, float thickness = 1.0f); // a: upper-left, b: lower-left (== upper-left + size), rounding_corners_flags: 4 bits corresponding to which corner to round
	IMGUI_API void AddRectFilled(const ImVec2 &p_min, const ImVec2 &p_max, ImU32 col, float rounding = 0.0f, ImDrawCornerFlags rounding_corners = ImDrawCornerFlags_All); // a: upper-left, b: lower-left (== upper-left + size)
	...
	// - "p_min" and "p_max" represent the upper-left and lower-left corners of the rectangle.
...
struct ImDrawData {
	...
	ImVec2 DisplaySize; // Size of the viewport to render (== io.DisplaySize for the main viewport) (DisplayPos + DisplaySize == lower-left of the orthogonal projection matrix to use)
...
// Common pitfalls:
// - If you pass a 'glyph_ranges' array to AddFont*** functions, you need to make sure that your array persist down until the
