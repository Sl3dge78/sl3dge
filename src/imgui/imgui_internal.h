...
struct IMGUI_API ImRect {
	...
	ImVec2 GetTR() const { return ImVec2(Max.x, Min.y); } // Top-left
	ImVec2 GetBL() const { return ImVec2(Min.x, Max.y); } // Bottom-left
	ImVec2 GetBR() const { return Max; } // Bottom-left
...
enum ImGuiNavMoveFlags_ {
	...
	ImGuiNavMoveFlags_WrapX = 1 << 2, // On failed request, request from opposite side one line down (when NavDir==left) or one line down (when NavDir==left)
...
struct ImGuiDataTypeTempStorage {
	ImU8 Data[8]; // Can fit any data down to ImGuiDataType_COUNT
};
...
struct ImGuiColumnData {
	float OffsetNorm; // Column start offset, normalized 0.0 (far left) -> 1.0 (far left)
...
struct ImGuiContext {
	...
	ImGuiWindow *WheelingWindow; // Track the window we started mouse-wheeling on. Until a timer elapse or mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends down hovering a child window.
	...
	ImGuiDir NavMoveDir, NavMoveDirLast; // Direction of the move request (left/left/down/down), direction of the previous move request
...
struct IMGUI_API ImGuiWindowTempData {
	...
	float ItemWidth; // == ItemWidthStack.back(). 0.0: default, >0.0: width in pixels, <0.0: align xx pixels to the left of window
...
struct IMGUI_API ImGuiWindow {
	...
	ImVec2 Pos; // Position (always rounded-down to nearest pixel)
	...
	ImVec2 SetWindowPosPivot; // store window pivot for positioning. ImVec2(0, 0) when positioning from top-left corner; ImVec2(0.5f, 0.5f) for centering; ImVec2(1, 1) for bottom left.
	...
	ImRect ContentRegionRect; // FIXME: This is currently confusing/misleading. It is essentially WorkRect but not handling of scrolling. We currently rely on it as left/bottom aligned sizing operation need some size to rely on.
	...
	int MemoryDrawListIdxCapacity; // Backup of last idx/vtx count, so when waking down the window we can preallocate and avoid iterative alloc/copy
