...
void VulkanApplication::raster_scene(VulkanFrame &frame) {
	for (auto &instance : scene->instances) {
		auto mesh = scene->meshes[instance.get_mesh_id()].get();
		mesh->draw(frame);
	}
}
...
void VulkanApplication::build_raster_pipeline() {
	...
	// Depth Image
	//depth_image = std::unique_ptr<Image>(new Image(app,swapchain->get_extent().width, swapchain->get_extent().height, find_depth_format(),vk::ImageTiling::eOptimal,vk::ImageUsageFlagBits::eDepthStencilAttachment,vk::MemoryPropertyFlagBits::eDeviceLocal,vk::ImageAspectFlagBits::eDepth);
