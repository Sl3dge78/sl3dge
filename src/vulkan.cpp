...
> VulkanPipeline. Starts with a default state, change it. Then build it. Holds :
	- VkPipeline
	- VkDescriptorSet
	- VkLayout
	- a ref to the renderpass
	...
	if (result < 0) {
		switch (result) {
			case VK_ERROR_EXTENSION_NOT_PRESENT:
				SDL_LogError(0, "VK_ERROR_EXTENSION_"
								"NOT_PRESENT");
				break;
			case VK_ERROR_LAYER_NOT_PRESENT:
				SDL_LogError(0, "VK_ERROR_LAYER_"
								"NOT_PRESENT");
				break;
			...
			case VK_ERROR_OUT_OF_POOL_MEMORY: {
				SDL_LogError(0, "VK_ERROR_OUT_OF_"
								"POOL_MEMORY");
			} break;
			case VK_ERROR_INITIALIZATION_FAILED: {
				SDL_LogError(0, "VK_ERROR_INITIALIZATION_"
								"FAILED");
			} break;
...
internal void GetQueuesId(VulkanContext *context) {
	...
	VkQueueFamilyProperties *queue_properties =
			(VkQueueFamilyProperties *)calloc(queue_count, sizeof(VkQueueFamilyProperties));
...
internal void CreateVkDevice(VkPhysicalDevice physical_device, const u32 graphics_queue,
		const u32 transfer_queue, const u32 present_queue, VkDevice *device) {
	// Queues
	...
	const char *extensions[] = { VK_KHR_SWAPCHAIN_EXTENSION_NAME,
		...
		VK_KHR_PIPELINE_LIBRARY_EXTENSION_NAME, VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME,
		VK_KHR_RAY_QUERY_EXTENSION_NAME, VK_KHR_SHADER_CLOCK_EXTENSION_NAME };
...
internal void CreateSwapchain(
		const VulkanContext *context, SDL_Window *window, Swapchain *swapchain) {
	...
	for (u32 i = 0; i < format_count; ++i) {
		...
		VkResult result = vkGetPhysicalDeviceImageFormatProperties(context->physical_device,
				formats[i].format, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL,
				VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_STORAGE_BIT, 0, &properties);
	...
	for (u32 i = 0; i < swapchain->image_count; i++) {
		DEBUGNameObject(context->device, (u64)swapchain->images[i], VK_OBJECT_TYPE_IMAGE,
				"Swapchain image");
		...
		AssertVkResult(vkCreateImageView(
				context->device, &image_view_ci, NULL, &swapchain->image_views[i]));
	}

	// Command buffers
	swapchain->command_buffers =
			(VkCommandBuffer *)calloc(swapchain->image_count, sizeof(VkCommandBuffer));
	...
	VkFenceCreateInfo ci = { VK_STRUCTURE_TYPE_FENCE_CREATE_INFO, NULL,
	...
	swapchain->image_acquired_semaphore =
			(VkSemaphore *)calloc(swapchain->image_count, sizeof(VkSemaphore));
...
internal void DestroySwapchain(const VulkanContext *context, Swapchain *swapchain) {
	vkFreeCommandBuffers(context->device, context->graphics_command_pool, swapchain->image_count,
			swapchain->command_buffers);
...
internal void CreateRenderPass(const VkDevice device, const Swapchain *swapchain,
...
internal void CreateShadowMapRenderPass(
		const VkDevice device, const Swapchain *swapchain, VkRenderPass *render_pass) {
	VkRenderPassCreateInfo render_pass_ci = {};
	render_pass_ci.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
	render_pass_ci.pNext = NULL;
	render_pass_ci.flags = 0;
...
internal void CreateShadowMapPipeline(const VkDevice device, const VkPipelineLayout layout,
	...
	VkVertexInputBindingDescription vtx_input_binding = { 0, sizeof(Vertex),
...
extern "C" __declspec(dllexport) VulkanContext *VulkanCreateContext(SDL_Window *window) {
	...
	GetQueuesId(context);
	CreateVkDevice(context->physical_device, context->graphics_queue_id, context->transfer_queue_id,
			context->present_queue_id, &context->device);
	...
	CreateBuffer(context->device, &context->memory_properties, sizeof(CameraMatrices),
	...
	CreateMultiSampledImage(context->device, &context->memory_properties, VK_FORMAT_D32_SFLOAT,
			...
			context->msaa_level, &context->depth_image);
	// MSAA Image
	CreateMultiSampledImage(context->device, &context->memory_properties, context->swapchain.format,
			{ context->swapchain.extent.width, context->swapchain.extent.height, 1 },
			VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, context->msaa_level, &context->msaa_image);
	...
	for (u32 i = 0; i < context->swapchain.image_count; ++i) {
		...
		AssertVkResult(vkCreateFramebuffer(
				context->device, &create_info, NULL, &context->framebuffers[i]));
	}
	...
	CreateShadowMapPipeline(context->device, context->shadowmap_layout.layout,
			context->shadowmap_render_pass, context->shadowmap_extent,
			&context->shadowmap_pipeline);

	CreateImage(context->device, &context->memory_properties, VK_FORMAT_D32_SFLOAT,
			context->shadowmap_extent,
			VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &context->shadowmap);
...
extern "C" __declspec(dllexport) void VulkanReloadShaders(VulkanContext *context, Scene *scene) {
	...
	CreateScenePipeline(context->device, scene->layout, &context->swapchain, context->render_pass,
			context->msaa_level, &scene->pipeline);
...
extern "C" __declspec(dllexport) void VulkanDrawFrame(
		VulkanContext *context, Scene *scene, GameData *game_data) {
	...
	result = vkAcquireNextImageKHR(context->device, swapchain->swapchain, UINT64_MAX,
			swapchain->image_acquired_semaphore[swapchain->semaphore_id], VK_NULL_HANDLE,
			&image_id);
	...
	const VkCommandBufferBeginInfo begin_info{ VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO, NULL, 0,
		NULL };
	...
	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, context->shadowmap_layout.layout,
			0, 1, &context->shadowmap_layout.descriptor_set, 0, NULL);
	for (u32 i = 0; i < scene->total_primitives_count; i++) {
		...
		vkCmdPushConstants(
				cmd, scene->layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstant), &push);
	...
	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, scene->layout, 0,
			scene->descriptor_set_count, scene->descriptor_sets, 0, NULL);
