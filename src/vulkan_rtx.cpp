...
internal void WriteRTXDescriptorSets(const VkDevice device, const VkDescriptorSet *descriptor_set, const VulkanRTXRenderer *pipeline, const VulkanContext *context) {
	VkDescriptorImageInfo img_info = {
		VK_NULL_HANDLE, pipeline->render_image.image_view, VK_IMAGE_LAYOUT_GENERAL
	};

	VkWriteDescriptorSetAccelerationStructureKHR as_write = {
		VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET_ACCELERATION_STRUCTURE_KHR, NULL, 1, &pipeline->TLAS
	};
...
internal void CreateRTXPipelineLayout(const VkDevice device, const Image *result_image, VulkanRTXRenderer *pipeline, GameCode *game) {
	// Set Layout
	...
	VkPushConstantRange push_constant_range = { VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 0, sizeof(PushConstant) };
...
void VulkanReloadRTXShaders(VulkanContext *context, VulkanRTXRenderer *renderer) {
	...
	CreateRtxPipeline(context->device, &renderer->layout, &context->rtx_properties, &renderer->pipeline);
}
// =========================
//
...
internal void CreateBLAS(VulkanContext *context, VulkanRTXRenderer *renderer, Buffer *vertex_buffer, const u32 vtx_count, Buffer *index_buffer, const u32 primitive_count) {
	...
	CreateBuffer(context, size_info.accelerationStructureSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &renderer->BLAS_buffer);
	DEBUGNameBuffer(context->device, &renderer->BLAS_buffer, "BLAS");
	...
	blas_ci.buffer = renderer->BLAS_buffer.buffer;
	blas_ci.offset = 0;
	...
	VkResult result = pfn_vkCreateAccelerationStructureKHR(context->device, &blas_ci, NULL, &renderer->BLAS);
	AssertVkResult(result);
	DEBUGNameObject(context->device, (u64)renderer->BLAS, VK_OBJECT_TYPE_ACCELERATION_STRUCTURE_KHR, "BLAS");

	// Now build it !!
	build_info.dstAccelerationStructure = renderer->BLAS;
...
internal void CreateBLASGLTF(VulkanContext *context, cgltf_mesh *mesh, Buffer *buffer, Buffer *BLAS_buffer, VkAccelerationStructureKHR *BLAS) {
	...
	//Alloc the blas buffer
	CreateBuffer(context, size_info.accelerationStructureSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, BLAS_buffer);
...
internal void CreateTLAS(VulkanContext *context, VulkanRTXRenderer *renderer) {
	...
	CreateBuffer(context, sizeof(VkAccelerationStructureInstanceKHR), VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &renderer->instance_data);
	...
	size_info.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR;
	pfn_vkGetAccelerationStructureBuildSizesKHR(context->device, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &build_info, &primitive_count, &size_info);
	CreateBuffer(context, size_info.accelerationStructureSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &renderer->TLAS_buffer);
...
void VulkanDrawRTXFrame(VulkanContext *context, VulkanRTXRenderer *pipeline) {
	u32 image_id;
...
VulkanRTXRenderer *VulkanCreateRTXRenderer(VulkanContext *context, GameCode *game) {
	VulkanRTXRenderer *renderer = (VulkanRTXRenderer *)malloc(sizeof(VulkanRTXRenderer));

	CreateRtxRenderImage(context->device, &context->swapchain, &context->memory_properties, &renderer->render_image);

	CreateRTXPipelineLayout(context->device, &renderer->render_image, renderer, game);
	CreateRtxPipeline(context->device, &renderer->layout, &context->rtx_properties, &renderer->pipeline);
	...
	// SHADER BINDING TABLE
	{
		...
		CreateBuffer(context, handle_size_aligned, VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &renderer->rgen_sbt);
		DEBUGNameBuffer(context->device, &renderer->rgen_sbt, "RGEN SBT");

		CreateBuffer(context, handle_size_aligned, VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &renderer->rmiss_sbt);
		DEBUGNameBuffer(context->device, &renderer->rmiss_sbt, "RMISS SBT");

		CreateBuffer(context, handle_size_aligned, VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &renderer->rchit_sbt);
		DEBUGNameBuffer(context->device, &renderer->rchit_sbt, "RCHIT SBT");

		u8 *data = (u8 *)malloc(sbt_size);

		VkResult result = pfn_vkGetRayTracingShaderGroupHandlesKHR(context->device, renderer->pipeline, 0, group_count, sbt_size, data);
		AssertVkResult(result);

		UploadToBuffer(context->device, &renderer->rgen_sbt, data, handle_size_aligned);
		UploadToBuffer(context->device, &renderer->rmiss_sbt, data + handle_size_aligned, handle_size_aligned * 2);
		UploadToBuffer(context->device, &renderer->rchit_sbt, data + (3 * handle_size_aligned), handle_size_aligned);

		renderer->strides[0] = { renderer->rgen_sbt.address, handle_size_aligned, handle_size_aligned };
		renderer->strides[1] = { renderer->rmiss_sbt.address, handle_size_aligned, handle_size_aligned };
		renderer->strides[2] = { renderer->rchit_sbt.address, handle_size_aligned, handle_size_aligned };
		renderer->strides[3] = { 0u, 0u, 0u };
		free(data);
	}
#if 0
    u32 vtx_count = 0;
	...
	CreateBuffer(context, data->buffers[0].size, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &renderer->gltf_buffer);
	...
	CreateBLASGLTF(context, &data->meshes[0], &renderer->gltf_buffer, &renderer->BLAS_buffer, &renderer->BLAS);

	cgltf_free(data);
#endif
	CreateTLAS(context, renderer);

	WriteRTXDescriptorSets(context->device, renderer->descriptor_sets, renderer, context);

	return renderer;
}

void VulkanDestroyRTXRenderer(VulkanContext *context, VulkanRTXRenderer *renderer) {
	DestroyBuffer(context->device, &renderer->gltf_buffer);

	DestroyBuffer(context->device, &renderer->TLAS_buffer);
	pfn_vkDestroyAccelerationStructureKHR(context->device, renderer->TLAS, NULL);
	DestroyBuffer(context->device, &renderer->instance_data);
#if 0
	...
	pfn_vkDestroyAccelerationStructureKHR(context->device, renderer->BLAS, NULL);
	...
	vkFreeDescriptorSets(context->device, renderer->descriptor_pool, 2, renderer->descriptor_sets);
	vkDestroyDescriptorPool(context->device, renderer->descriptor_pool, NULL);

	vkDestroyDescriptorSetLayout(context->device, renderer->app_set_layout, NULL);
